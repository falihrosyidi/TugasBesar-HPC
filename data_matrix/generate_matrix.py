...
def generate_matrices(N):
    ...
    matrix_A.tofile(f"matrix_A_{N}.bin")
    matrix_B.tofile(f"matrix_B_{N}.bin")
    matrix_I.tofile(f"matrix_I_{N}.bin")
    print(f"Saved binary files: matrix_A_{N}.bin, matrix_B_{N}.bin, matrix_I_{N}.bin")
    
    # 4. Save as CSV (For your visual inspection)
    np.savetxt(f"matrix_A_{N}.csv", matrix_A, delimiter=",", fmt="%.4f")
    np.savetxt(f"matrix_B_{N}.csv", matrix_B, delimiter=",", fmt="%.4f")
    # Using %.0f for the identity matrix CSV since it only contains whole 1s and 0s
    np.savetxt(f"matrix_I_{N}.csv", matrix_I, delimiter=",", fmt="%.0f") 
    print(f"Saved CSV files: matrix_A_{N}.csv, matrix_B_{N}.csv, matrix_I_{N}.csv")
